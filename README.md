...
- **事件拆分尚未实现**：当前主节点通常还是一个 Q/A 窗口，不是真正的人类式事件片段。
- **情景共现不完整**：`activate()` 可以建共现边，但自动事件窗口和长期情景结构还没成型。
- **抽象查询可能漂移**：概念图能拉近语义，但 Top 1 排序还可能被抽象相似记忆抢走。
- **构建阶段偏慢**：145 条 Q/A 的概念图构建约 150s 量级，后续需要继续做增量和缓存优化。
- **NLP 可选依赖需要隔离**：spaCy TRF 等实验依赖可能和 Qwen3 / transformers 版本产生冲突，当前不建议作为默认安装路径。
...
1. 增加候选门控或 rerank 模式，减少概念图过度激活。
2. 优化概念图增量构建。
3. 固化 benchmark 查询集和人工判断口径。
4. 文档化推荐依赖环境。
